# 每周潜力项目汇总 - 报告生成 + 同步网站 + 同步 Obsidian
...
try {
  ...
  Get-Content -LiteralPath "C:\Users\Administrator\Documents\Github\scripts\weekly-report-prompt.txt" -Encoding UTF8 -Raw | codex exec -C $root -s danger-full-access --skip-git-repo-check 2>&1 | Out-Null
  Log "[1/3] 完成"
  Log "[2/3] 同步网站..."
